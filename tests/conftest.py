...
Responsibilities:
  ...
  3. Insert supplement_scripts/ onto sys.path so the standalone scripts
...
# ── 3. supplement_scripts/ on sys.path ───────────────────────────────────────
# The scripts in supplement_scripts/ are standalone (no __init__.py).
# Adding the directory lets tests do `from averaging import load_and_melt`
# etc. without a package wrapper.
SUPPLEMENT_DIR = PROJECT_ROOT / "supplement_scripts"
