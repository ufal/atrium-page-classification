...
|:----------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:------------|
| **v1.7.5-beta** | Re-vendored `atrium_document.py`: `set_source()` now fills sub-keys the first writer left unset, so a partial first write no longer silently swallows a later `origin=` and §1a's deferred origin checks actually resolve. The four `sys.modules["atrium_document"] = MagicMock()` masks are gone — they made whether the adapter tested the real `DocumentRecord` or a stub depend on pytest's alphabetical collection order — and `test_the_real_module_is_under_test` now guards their return. `scheduled-smoke.yml` no longer cancels in progress (the only nightly of the five that did); `codeql.yml` / `security.yml` scope their concurrency group by `github.event_name` so a push cannot kill the cron; `release.yml` declares one. Bumps: ultralytics, PyMuPDF. | Pre-release |
| **v1.7.4-beta** | Re-vendored `atrium_document.py`: `DocumentRecord` now **inherits `doc_id` from the baseline** instead of overwriting it with the caller's derivation, so no stage can re-key an accreted record. Nothing changes for this tool's own behaviour: `doc_id_and_page()` already composed `canonical_doc_id()` with the page-label split and agreed with the rest of the pipeline. Also `service/document_json.py` split the per-page image (`/predict_image`) and whole-document (`/predict_document`) upload paths onto one shared derivation, `tests/test_document_originators.py` -> canonical shared set, dependencies bumped.                                                                                                                                            | Pre-release |
| **v1.7.3-beta** | End-to-end GHA pipeline for JSON input-output by `atrium_document` standard is refined, and tested for the draft JSON schema design.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Pre-release |
| **v1.7.2-beta** | Major GHA workflows update - references to `@v1` on the hub repo tag added in workflows.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Pre-release |
| **v1.7.1-beta** | CI/CD hardening release. Release bundle now ships the nine first-party modules `run.py` and the service reached transitively, plus an AST-based guard that fails the build if the bundle is ever incomplete again. Workflows migrated to the hub's reusable templates pinned at `@v1`; version consistency enforced by the vendored `check_version.py`. Dependency bumps.                                                                                                                                                                                                                                                                                                                                                                                                  | Pre-release |
| **v1.7.0-beta** | `atrium_document` standard of input in JSON format and output integrated. Aligned paradata template with the hub.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Pre-release |
| **v1.6.0-beta** | Update of the latest versions to `vX.4` for the licensed dataset models. OpenAPI standards for service applied - draft. Tests fixes. Updates and bumping of dependency versions. Edited GHA release workflow.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Pre-release |
| **v1.5.1-beta** | Added licenses test according to the template. Fixed auto version reading. Updated dependency versions. Added new tests. Fixes according to Fable review.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Pre-release |
| **v1.5.0-beta** | Added folds_csv argument insted of by-seed selection for re-training and re-evaluation of the models on the cropped dataset. Updated paradata-related code with a template for all 4 atrium-project repos. Added agent_dev_logs instead of issue comments, containing issues and their digests+plans.                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Pre-release |
| **v1.4.3-beta** | Dataset link updated to reference licensed data and Docker GHA alignemnent continues                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Pre-release |
| **v1.4.2-beta** | Ruff checks applied but secret-scan removed - Docker GH actions alignemnent                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Pre-release |
| **v1.4.1-beta** | Next round of LLM review edits and Docker GH actions alignemnent                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Pre-release |
| **v1.4.0-beta** | Organization of files into sub-directories and LLM review edits of the source code                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Pre-release |
| **v1.3.0-beta** | Memory-aware grouped parallel execution for `--best` inference added, Docker wrapper and API service aligned                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Pre-release |
| **v1.2.1-beta** | Docker wrapper added (draft, untested version)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Pre-release |
| **v1.2.0-beta** | YOLO-cls models added to entry point. Paradata logging was extended with model licenses resolution                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Pre-release |
| **v1.1.0-beta** | Image samples replaced with CC BY-NC 4.0 licensed images from various sources, data-processing scripts were finalized with their own unit tests and wide-scale adaptation of the CLI flags for input-output arguments                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Pre-release |                                                                                                                                                                                                                                                                         | Pre-release |
| **v1.0.4-beta** | Testing & docs housekeeping (Pytest unit tests added for core pipeline components, CONTRIBUTING.md expanded with testing guide and full release history section)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Pre-release |
| **v1.0.3-beta** | BETA release of the final program (Empty CLASS-SCORE pairs feature added to averaged results output, Reduced-size example results added for Prague and Brno collections, CONTRIBUTING.md extended with user contribution and pre-release history, Data licence added for shared page image samples)                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Pre-release |
