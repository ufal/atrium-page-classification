...
def main(argv=None):
    ...
    # [Phase 1 / G1] Heavy imports deferred
    import numpy as np
    import pandas as pd
    ...
    if args.train:
        if args.yolo:
            ...
            if args.folds_csv:
                ...
                top1_preds = [p[0][0] if isinstance(p, list) else p for p in test_predictions]

                print("=" * 40)
                print(
                    f"TEST SET's correct percentage:\t{round(100 * sum([1 for true, pred in zip(test_labels_indices, top1_preds) if true == pred]) / len(test_labels_indices), 2)}%"
                )
                print("=" * 40)
                print(
        ...
        if args.dir or args.directory is not None:
            ...
            if not args.best:
                if not chunked_result_record:  # all at once (no chunking)
                    ...
                    rdf.sort_values(["FILE", "PAGE"], ascending=[True, True], inplace=True)
                    top_out_path = f"{output_dir}/tables/{time_stamp}_{revision_model_name_local}_TOP-{top_N}.csv"
                    rdf.to_csv(
                        top_out_path,
                        sep=",",
                        index=False,
