...
def resolve_base_model(revision: str, explicit_base: str = None) -> str:
    ...
    for key, base_model in REVISION_TO_BASE_MODEL.items():
        if revision.startswith(key):
            return base_model
    raise ValueError(
        f"Base model not found for revision: {revision} (pass -b/--base explicitly)"
    )
...
def load_or_download(
    ...
    classifier = ImageClassifier(
        checkpoint=base_model, num_labels=len(def_categ), store_dir=str(cp_dir)
    )
...
def ensure_best_models(
    model_dir: str, cp_dir: str, hf_repo: str, allow_download: bool
) -> None:
    ...
    for rev in missing:
        clf = load_or_download(
            rev,
            REVISION_BEST_MODELS[rev],
            model_dir,
            cp_dir,
            hf_repo,
            allow_download=True,
...
if __name__ == "__main__":
    ...
    top_N = config.getint(
        "SETUP", "top_N"
    )  # top N predictions, 3 is enough, 11 for "raw" scores

    config_base_model = config.get(
        "SETUP", "base_model"
    )  # explicit -b/--base override wins
    ...
    parser = argparse.ArgumentParser(
        description="Page sorter based on ViT / RegNetY / EffNetV2 (inference-only)"
    )
    parser.add_argument(
        "-f", "--file", type=str, default=None, help="Single page image path"
    )
    parser.add_argument(
        "-d",
        "--directory",
        type=str,
        default=None,
        help="Path to folder with unprocessed pages",
    )
    parser.add_argument(
        "--dir",
        help=f"Process the configured input folder ({config_input_dir})",
        action="store_true",
    )
    parser.add_argument(
        "-b",
        ...
        type=str,
        default=None,
        help="Repository of the base model (override)",
    )
    parser.add_argument(
        "-rev",
        "--revision",
        type=str,
        default=hf_version,
        help="HF revision tag of the fine-tuned model",
    )
    parser.add_argument(
        "-tn",
        "--topn",
        type=int,
        default=top_N,
        help="Number of top predictions per page",
    )
    parser.add_argument(
        "--hf",
        help="Download missing models from the HF hub",
        action="store_true",
        default=HF,
    )
    parser.add_argument(
        "--raw",
        help="Also save raw per-category scores CSV",
        action="store_true",
        default=raw,
    )
    parser.add_argument(
        ...
        help="Image file format for directory scraping",
    )
    parser.add_argument(
        "--best",
        help="Run the best-5 models ensemble with averaging",
        action="store_true",
    )
    parser.add_argument(
        "--parallel",
        help="Memory-aware parallel loading of the best models (CUDA only)",
        action="store_true",
    )
    parser.add_argument(
        "--no-average-best",
    ...
    parser.add_argument(
        "--save_intermediates",
        help="Save per-model TOP-N CSVs next to the averaged result",
        action="store_true",
    )
    ...
    test_images: list = []
    input_dir = (
        Path(config_input_dir) if args.directory is None else Path(args.directory)
    )
    ...
    if args.file is not None:
        ...
        test_images = sorted(
            str(p) for p in directory_scraper(Path(input_dir), args.file_format)
        )
        if not test_images:
        ...
        load_or_download(
            args.revision, base_model, model_dir, cp_dir, hf_repo, allow_download=True
        )
        print(
            f"Download-only mode: model {args.revision} is ready under {model_dir}. Exiting."
        )
        sys.exit(0)
        ...
        print(
            f"Download-only mode: all best-5 models are ready under {model_dir}. Exiting."
        )
        sys.exit(0)
    else:
        parser.error(
            "Provide -f FILE or -d DIR (or --dir), or use --hf for download-only mode."
        )
    ...
    # ── Single-model path ────────────────────────────────────────────────────
    base_model = resolve_base_model(args.revision, args.base)
    classifier = load_or_download(
        args.revision, base_model, model_dir, cp_dir, hf_repo, allow_download=args.hf
    )
    ...
    test_predictions, raw_prediction = classifier.infer_dataloader(
        dataloader, top_n=args.topn, raw=args.raw
    )
    ...
    rdf, raw_df = dataframe_results(
        test_images, test_predictions, def_categ, args.topn, raw_prediction
    )
